...
def process_transcription_direct(job_id: str, file_path: str, model: str = "openai-whisper", set_list: str = "", custom_prompt: str = ""):
    ...
    def transcribe():
        try:
            ...
            print(f"DEBUG: Transcription check: model in ['openai-whisper', 'whisper-plus-gemini'] = {model in ['openai-whisper', 'whisper-plus-gemini']}")
            print(f"DEBUG: Analysis check: model in ['whisper-plus-gemini', 'gemini-analysis-only'] = {model in ['whisper-plus-gemini', 'gemini-analysis-only']}")
